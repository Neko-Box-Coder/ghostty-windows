...
fn currentWindowsDirectory(alloc: Allocator) !?[]const u8 {
    return std.process.currentPathAlloc(global.io(), alloc) catch |err| switch (err) {
...
fn lookupExecutable(alloc: Allocator, exe: []const u8) !?[]u8 {
    const envMap = try global.environMap();
    return try internal_os.path.expand(global.io(), alloc, &envMap, exe);
}
...
fn defaultWindowsHome(_: Allocator) !?[]const u8 {
