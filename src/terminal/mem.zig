...
pub inline fn canReclaim(comptime mode: DecommitMode) bool {
    ...
    return switch (mode) {
        .zero, .strict => supported: {
            ...
            // Windows provides DiscardVirtualMemory (Win8.1+), which
            // immediately releases the physical pages behind a committed
            // range while keeping it committed and accessible; touched
            // pages fault back in on demand.
...
pub fn decommit(
    ...
    // DiscardVirtualMemory immediately releases the physical pages behind a
    // committed read-write range while keeping it committed and accessible.
    // Zero mode clears its dirty prefix first because the OS documents the
    // post-discard contents as undefined: pages it actually discarded fault
    // back as zeroes, and pages it could not discard keep the zeroes we
    // wrote (mirroring the Darwin FREE_REUSABLE path above).
    if (comptime builtin.os.tag == .windows) {
        if (comptime mode == .zero) @memset(memory[0..dirty_len], 0);

        const rc = kernel32.DiscardVirtualMemory(memory.ptr, memory.len);
        if (rc == 0) return true;
        log.warn("DiscardVirtualMemory failed err={}", .{rc});
        // Zero mode already cleared its dirty prefix above, so the content
        // guarantee holds even though no memory was reclaimed.
        return false;
    }
...
/// Windows-only externs used by decommit. Declared locally so the terminal
/// platform layer stays self-contained for libghostty-vt builds.
const kernel32 = struct {
    /// Returns a win32 error code; 0 (ERROR_SUCCESS) means the physical
    /// pages were discarded. The range stays committed either way.
    extern "kernel32" fn DiscardVirtualMemory(
...
///
/// Linux, Windows, and test builds need no explicit operation. Darwin pairs
