                ...
                .fork_exec => |cmd| {
                    if (cmd.pid) |handle| {
                        ...
                        if (exec.process_watcher_thread) |t|
                            t.setName(global.io(), "proc-watcher") catch {};
                    }
                },
                else => {},
            }
...
fn processExitWindows(
    ...
    const result = windows.exp.kernel32.WaitForSingleObject(process_handle, windows.INFINITE);
    ...
    if (windows.exp.kernel32.GetExitCodeProcess(process_handle, &exit_code) == windows.FALSE) {
